 ...
 * \param read_buf_size popen read buf. Also serves as a minimum size for the
 * dynamically allocated returned output if not already allocaded
 * \param nstr_output Pointer to a valid N_STR or NULL (see read_buf_size)
 * \param ret Command output if any. Should be initialized to -1 and tested
 * against to check if it's valid.
 * \return TRUE or FALSE
 */
int n_popen(char *cmd, int read_buf_size, void **nstr_output, int *ret)
{
    ...
    int err = errno;
    if (fp == NULL) {
        n_log(LOG_ERR, "popen( %s ) returned NULL , %s (%d)", cmd,
              strerror(err), err);
        return FALSE;
    }
    ...
    err = errno;
    if (status == -1) {
        n_log(LOG_ERR, "pclose( %s ) returned -1 , %s (%d)", cmd, strerror(err),
              err);
        if (WIFEXITED(status)) {
...
{
    return n_daemonize_ex(N_DAEMON_NO_SIGCHLD_IGN |
                          N_DAEMON_NO_SIGCHLD_HANDLER);
}

/*!\fn n_daemonize_ex( int mode )
 *\brief Daemonize program
 *\param mode flag made of a combination of N_DAEMON_NO_CLOSE,
 * N_DAEMON_NO_STD_REDIRECT, N_DAEMON_NO_DOUBLE_FORK , N_DAEMON_NO_SETSID,
 * N_DAEMON_NO_UMASK, N_DAEMON_NO_CHDIR, N_DAEMON_NO_SETSID,
 * N_DAEMON_NO_SIGCHLD_IGN, N_DAEMON_NO_SIGCHLD_HANDLER or 0 for defaults
 *\return TRUE or FALSE
 */
int n_daemonize_ex(int mode)
{
    ...
    if (!(mode & N_DAEMON_NO_SETSID)) {
        ...
        if (setsid() < 0) {
            error = errno;
            n_log(LOG_ERR,
                  "Error: unable to set session leader with setsid(): %s",
                  strerror(error));
            exit(1);
        } else {
    ...
    if (!(mode & N_DAEMON_NO_DOUBLE_FORK)) {
        ...
        error = errno;
        if (pid < 0) {
            n_log(LOG_ERR, "Error: unable to double fork child: %s",
                  strerror(error));
            exit(1);
        }
        if (pid > 0) {
    ...
    if (!(mode & N_DAEMON_NO_CLOSE)) {
        /* Close all open file descriptors except standard ones (stdin, stdout,
         * stderr) */
...
pid_t system_nb(const char *command, int *infp, int *outfp)
{
    ...
    if (pid == 0) {
        ...
        int error = errno;
        n_log(LOG_ERR, "%s:%d: exec failed: %s", __FILE__, __LINE__,
              strerror(error));
        exit(42);
    }
    if (infp != NULL) {
