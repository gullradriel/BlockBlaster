...
#else
/*! fall through macro for switch cases, avoid warning at compilation */
#define FALL_THROUGH /* fall through */ ((void) 0)
#endif /* __GNUC__ >= 7 */
...
#define Malloc(__ptr, __struct, __size)                                        \
    {                                                                          \
        ...
        if (!__ptr) {                                                          \
            n_log(                                                             \
                LOG_ERR,                                                       \
                "( %s *)calloc( %ld , sizeof( %s ) ) %s at line %d of %s \n",  \
                #__ptr, __size, #__struct,                                     \
                (__n_errno == 0) ? "malloc error" : strerror(__n_errno),       \
                __LINE__, __FILE__);                                           \
        }                                                                      \
    }
    ...
    {                                                                          \
        ...
        if (!__ptr) {                                                          \
            n_log(LOG_ERR, "%s=alloca( %d ) %s at line %d of %s \n", #__ptr,   \
                  __size,                                                      \
                  (__n_errno == 0) ? "alloca error" : strerror(__n_errno),     \
                  __LINE__, __FILE__);                                         \
        } else {                                                               \
...
#define Realloc(__ptr, __struct, __size)                                       \
    {                                                                          \
        ...
        if (!__new_ptr) {                                                      \
            n_log(                                                             \
                LOG_ERR,                                                       \
                "( %s *)realloc( %s * sizeof( %d ) ) %s at line %d of %s \n",  \
                #__ptr, #__struct, __size,                                     \
                (__n_errno == 0) ? "realloc error" : strerror(__n_errno),      \
                __LINE__, __FILE__);                                           \
        } else {                                                               \
            __ptr = __new_ptr;                                                 \
        }                                                                      \
...
#define Reallocz(__ptr, __struct, __old_size, __size)                          \
    {                                                                          \
        ...
        if (!__new_ptr) {                                                      \
            ...
            __ptr = __new_ptr;                                                 \
            if (__byte_size > __old_size)                                      \
                memset((__ptr + __old_size), 0, __byte_size - __old_size);     \
        }                                                                      \
    }
...
#define Free(__ptr)                                                            \
    if (__ptr) {                                                               \
        ...
        n_log(LOG_DEBUG,                                                       \
              "Free( %s ) already done or NULL at line %d of %s \n", #__ptr,   \
              __LINE__, __FILE__);                                             \
    }
...
#define __n_assert(__ptr, __ret)                                               \
    if (!(__ptr)) {                                                            \
        n_log(LOG_DEBUG, "if( !(%s) ) assert at line %d of %s", #__ptr,        \
              __LINE__, __FILE__);                                             \
        __ret;                                                                 \
    }
...
#define checkerror()                                                           \
    if (___error__check_flag == TRUE) {                                        \
        n_log(LOG_ERR, "checkerror return false at line %d of %s", __LINE__,   \
              __FILE__);                                                       \
        goto error;                                                            \
    }
...
#define init_lock(__rwlock_mutex)                                              \
    ({                                                                         \
        ...
        do {                                                                   \
            ...
            if (__ret != 0) {                                                  \
                n_log(LOG_ERR, "Error %s while initializing %s at %s:%s:%d",   \
                      strerror(__ret), #__rwlock_mutex, __FILE__, __func__,    \
                      __LINE__);                                               \
            }                                                                  \
...
#define read_lock(__rwlock_mutex)                                              \
    ({                                                                         \
        ...
        do {                                                                   \
            ...
            if (__ret != 0) {                                                  \
                n_log(LOG_ERR, "Error %s while read locking %s at %s:%s:%d",   \
                      strerror(__ret), #__rwlock_mutex, __FILE__, __func__,    \
                      __LINE__);                                               \
            }                                                                  \
        } while (0);                                                           \
...
#define write_lock(__rwlock_mutex)                                             \
    ({                                                                         \
        ...
        do {                                                                   \
            ...
            if (__ret != 0) {                                                  \
                n_log(LOG_ERR, "Error %s while write locking %s at %s:%s:%d",  \
                      strerror(__ret), #__rwlock_mutex, __FILE__, __func__,    \
                      __LINE__);                                               \
            }                                                                  \
        } while (0);                                                           \
...
#define unlock(__rwlock_mutex)                                                 \
    ({                                                                         \
        ...
        do {                                                                   \
            ...
            if (__ret != 0) {                                                  \
                n_log(LOG_ERR, "Error %s while unlocking %s at %s:%s:%d",      \
                      strerror(__ret), #__rwlock_mutex, __FILE__, __func__,    \
                      __LINE__);                                               \
            }                                                                  \
        } while (0);                                                           \
...
#define rw_lock_destroy(__rwlock_mutex)                                        \
    ({                                                                         \
        ...
        do {                                                                   \
            ...
            if (__ret != 0) {                                                  \
                n_log(LOG_ERR, "Error %s while destroying %s at %s:%s:%d",     \
                      strerror(__ret), #__rwlock_mutex, __FILE__, __func__,    \
                      __LINE__);                                               \
            }                                                                  \
        } while (0);                                                           \
...
/**
 * \brief Print a formatted error message to stderr and abort the process.
 *
...
/**
 * \brief Retrieve the host name of the machine.
 *
 * \param computer_name  Buffer to receive the null-terminated host name.
...
/**
 * \brief Return the name of the running executable.
 *
 * \return  Pointer to a statically allocated string containing the program
 ...
 *
 * \return  Pointer to a statically allocated string containing the directory
 *          path, or NULL on error.
 */
char *get_prog_dir(void);

/**
 * \brief Test whether a file exists and is accessible.
 *
...
/**
 * \brief Run a shell command and capture its output into an N_STR.
 *
 * \param cmd           Null-terminated shell command to execute.
 * \param read_buf_size Internal read buffer size in bytes.
 * \param nstr_output   Receives a pointer to the N_STR containing the output.
 * \param ret           Receives the exit status of the command.
 * \return              TRUE on success, FALSE on failure.
 */
int n_popen(char *cmd, int read_buf_size, void **nstr_output, int *ret);

/**
 * \brief Log all significant environment variables at the given log level.
 *
 * \param loglevel  Log severity level (LOG_EMERG .. LOG_DEBUG).
 */
void log_environment(int loglevel);

#ifndef __windows__
/**
 * \brief SIGCHLD signal handler that reaps zombie child processes.
 *
 * \param sig  Signal number (should be SIGCHLD).
 */
void sigchld_handler(int sig);

/**
 * \brief Install the sigchld_handler() as the SIGCHLD handler.
 *
 * \return  0 on success, -1 on failure.
 */
int sigchld_handler_installer();

/** \brief Daemonize flag: do not close file descriptors. */
#define N_DAEMON_NO_CLOSE 2
/** \brief Daemonize flag: do not redirect stdin/stdout/stderr to /dev/null. */
#define N_DAEMON_NO_STD_REDIRECT 4
/** \brief Daemonize flag: do not perform the second fork. */
#define N_DAEMON_NO_DOUBLE_FORK 8
/** \brief Daemonize flag: do not call setsid(). */
#define N_DAEMON_NO_SETSID 16
/** \brief Daemonize flag: do not call umask(0). */
#define N_DAEMON_NO_UMASK 32
/** \brief Daemonize flag: do not call chdir("/"). */
#define N_DAEMON_NO_CHDIR 64
/** \brief Daemonize flag: do not set SIGCHLD to SIG_IGN. */
#define N_DAEMON_NO_SIGCHLD_IGN 128
/** \brief Daemonize flag: do not install the internal zombie SIGCHLD handler.
 */
#define N_DAEMON_NO_SIGCHLD_HANDLER 256

/**
 * \brief Daemonize the current process with default options.
 *
 * \return  0 on success, -1 on failure.
 */
...
/**
 * \brief Daemonize the current process with configurable options.
 *
 * \param mode  Bitmask of N_DAEMON_* flags controlling which steps to skip.
 * \return      0 on success, -1 on failure.
 */
int n_daemonize_ex(int mode);

/**
 * \brief Launch a child process with non-blocking stdin/stdout pipes.
 *
 * \param command  Shell command to execute.
 * \param infp     Receives the file descriptor for the child's stdin, or NULL.
 * \param outfp    Receives the file descriptor for the child's stdout, or NULL.
...
/**
 * \brief Reconstruct a hidden string from variadic obfuscated byte arguments.
 *
 * \param buf  Output buffer to receive the reconstructed string.
 * \param ...  Obfuscated byte values terminated by a sentinel.
 */
void N_HIDE_STR(char *buf, ...);

/**
 * \brief Return a pointer to the file extension within a path string.
 *
 * \param path  File path string.
 * \return      Pointer to the extension (including the dot), or an empty string
