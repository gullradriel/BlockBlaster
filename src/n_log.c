...
/*! array of log levels */
static LOG_LEVELS prioritynames[] = {{"EMERG", LOG_EMERG, "ERROR"},
 ...
 *\param log_level Log level value. Supported:
 * NOLOG,LOG_NOTICE/INFO/ERR/DEBUG,LOG_FILE/STDERR/SYSJRNL
 */
 ...
 *\brief compute the size of a string made with format 'fmt' and arguments in
 * the va_list 'ap', helper for vasprintf
...
int vasprintf(char **strp, const char *fmt, va_list ap)
{
    ...
    char *str = NULL;
    Malloc(str, char,
           (size_t) len + 1 + sizeof(void *)); /* len + EndOfString + padding */
    if (!str)
        return -1;
    int r = vsnprintf(str, (size_t) (len + 1), fmt,
                      ap); /* "secure" version of vsprintf */
    if (r == -1)
        return free(str), -1;
    *strp = str;
    return r;
...
/*!\fn void _n_log( int level , const char *file , const char *func , int line ,
 * const char *format , ... )
 *\brief Logging function. log( level , const char *format , ... ) is a macro
 * around _log
            ...
            const char *format, ...)
{
    ...
    if (level <= log_level) {
        ...
        case LOG_SYSJRNL:
            ...
            va_end(args);
#ifdef __windows__
            needed = (unsigned long long) snprintf(
                NULL, 0,
                "start /B EventCreate /t %s /id 666 /l APPLICATION /so %s /d "
                "\"%s\" > NUL 2>&1",
                prioritynames[level].w_name, name, syslogbuffer);
            Malloc(eventbuffer, char, needed + 4);
            sprintf(eventbuffer,
                    "start /B EventCreate /t %s /id 666 /l APPLICATION /so %s "
                    "/d \"%s\" > NUL 2>&1",
                    prioritynames[level].w_name, name, syslogbuffer);
            system(eventbuffer);
#else
            ...
            break;
        default:
            fprintf(out, "%s:%jd:%s->%s:%d ", prioritynames[level].c_name,
                    (intmax_t) time(NULL), file, func, line);
            va_start(args, format);
