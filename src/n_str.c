 ...
 *\brief trim and zero end the string, WARNING: keep and original pointer to
 * delete the string correctly
...
char *nfgets(char *buffer, NSTRBYTE size, FILE *stream)
{
    ...
    if (it == (size - 1)) {
        n_log(LOG_DEBUG, "buffer %p size %d fully filled by fgets on stream %p",
              buffer, size, stream);
    }
...
int char_to_nstr_ex(const char *from, NSTRBYTE nboct, N_STR **to)
{
    if ((*to)) {
        n_log(LOG_ERR,
              "destination N_STR **str is not NULL (%p), it contain (%s). You "
              "must provide an empty destination.",
              (*to), ((*to) && (*to)->data) ? (*to)->data : "DATA_IS_NULL");
    ...
    /* added a sizeof( void * ) to add a consistant and secure padding at the
     * end */
 ...
 *\brief Convert a char into a N_STR, direct use of linked source pointer, short
 * version
 *\param src A char *string to convert
 *\return A N_STR copy of src or NULL
 */
 ...
 *\brief Load a whole file into a N_STR. Be aware of the NSTRBYTE addressing
 * limit (2GB commonly)
...
N_STR *file_to_nstr(char *filename)
{
    ...
    __n_assert(filename,
               n_log(LOG_ERR, "Unable to make a string from NULL filename");
               return NULL);
...
#endif
            n_log(LOG_ERR, "Couldn't stat %s. Errno: %s", filename,
                  strerror(errno));
#ifdef __linux__
        }
#endif
    ...
    if ((filestat.st_size + 1) >= (pow(2, 32) - 1)) {
        n_log(LOG_ERR, "file size >= 2GB is not possible yet. %s is %lld oct",
              filename, filestat.st_size);
        return NULL;
    }

    n_log(LOG_DEBUG, "%s file size is: %lld", filename,
    ...
    __n_assert(tmpstr, fclose(in);
               n_log(LOG_ERR, "Unable to get a new nstr of %ld octets",
                     filestat.st_size + 1);
               return NULL);
...
int nstr_to_fd(N_STR *str, FILE *out, int lock)
{
    ...
    if ((written_to_file = fwrite(str->data, sizeof(char), str->written,
                                  out)) != (size_t) str->written) {
        n_log(LOG_ERR, "Couldn't write file, fwrite %d of %d octets",
              written_to_file, str->written);
        ret = FALSE;
    }
    if (ferror(out)) {
        n_log(LOG_ERR, "There were some errors when writing to %d",
              fileno(out));
        ret = FALSE;
    }
...
int nstr_to_file(N_STR *str, char *filename)
{
    ...
    __n_assert(out, n_log(LOG_DEBUG, "Couldn't open %s", _str(filename));
               return FALSE);
...
/*!\fn int str_to_int_ex( const char *s , NSTRBYTE start , NSTRBYTE end , int
 *i, const int base )
                  ...
                  const int base)
{
    ...
    __n_assert(
        tmpstr,
        n_log(LOG_ERR,
              "Unable to Malloc( tmpstr , char ,  sizeof( int ) + %d - %d )",
              end, start);
        return FALSE);
...
/*!\fn int str_to_int_nolog( const char *s, NSTRBYTE start, NSTRBYTE end, int
 *i, const int base, N_STR **infos )
 * \brief Helper for string[start to end] to integer. Automatically add /0 for
 conversion. Leave values untouched if any error occur. Work on a copy of the
 chunk.
 * \param s String to convert
 ...
 * \param i A pointer to an integer variable which will receieve the value.
 * \param base Base for converting values
 * \param infos If not NULL , contain the errors. remember to free the pointer
 if returned !!
 * \return TRUE or FALSE
 */
                     ...
                     const int base, N_STR **infos)
{
    ...
    l = strtol(tmpstr, &endstr, base);
    if ((errno == ERANGE && l == LONG_MAX) || l > INT_MAX) {
        nstrprintf((*infos), "OVERFLOW reached when converting %s to int",
                   tmpstr);
        Free(tmpstr);
        return FALSE;
    }
    if ((errno == ERANGE && l == LONG_MIN) || l < INT_MIN) {
        nstrprintf((*infos), "UNDERFLOW reached when converting %s to int",
                   tmpstr);
        Free(tmpstr);
        return FALSE;
    }
...
/*!\fn int str_to_long_ex( const char *s , NSTRBYTE start , NSTRBYTE end , long
 int *i, const int base )
 * \brief Helper for string[start to end] to long integer. Automatically add /0
 for conversion. Leave values untouched if any error occur. Work on a copy of
 the chunk.
 * \param s String to convert
 * \param start Start position of the chunk
 * \param end End position of the chunk
                   ...
                   const int base)
{
    ...
    if (tmpstr == endstr) {
        n_log(LOG_ERR, " number : %lu  invalid  (no digits found, 0 returned)",
              l);
        Free(tmpstr);
        return FALSE;
    ...
    } else if (error == EINVAL) /* not in all c99 implementations - gcc OK */
    {
        ...
        n_log(LOG_ERR, " number : %lu  invalid  (unspecified error occurred)",
              l);
        Free(tmpstr);
        return FALSE;
    } else if (error == 0 && tmpstr && !*endstr) {
        n_log(LOG_DEBUG,
              " number : %lu    valid  (and represents all characters read)",
              l);
...
/*!\fn int str_to_long_long_ex( const char *s, NSTRBYTE start, NSTRBYTE end,
 * long long int *i, const int base )
 * \brief Helper for string[start to end] to long long integer. Automatically
 * add /0 for conversion. Leave values untouched if any error occur. Work on a
 * copy of the chunk.
 * \param s String to convert
 * \param start Start position of the chunk
 * \param end End position of the chunk
                        ...
                        long long int *i, const int base)
{
    ...
    {
        n_log(LOG_ERR, "number: '%s' invalid (base contains unsupported value)",
              s);
        Free(tmpstr);
        return FALSE;
    } else if (error != 0 && l == 0) {
        ...
        n_log(LOG_DEBUG, " number : '%llu' valid (remaining characters: '%s')",
              l, s);
    }
    Free(tmpstr);
    *i = l;
 ...
 *\brief skip while 'toskip' occurence is found from 'iterator' to the next non
 * 'toskip' position. The new iterator index is automatically stored, returning
 * to it first value if an error append.
...
int skipw(char *string, char toskip, NSTRBYTE *iterator, int inc)
{
    ...
    // previous = *iterator;
    if (toskip == ' ') {
        while (*iterator <= (NSTRBYTE) strlen(string) &&
               isspace(string[*iterator])) {
            if (inc < 0 && *iterator == 0) {
                error_flag = 1;
                break;
            } else
 ...
 *\brief skip until 'toskip' occurence is found from 'iterator' to the next
 * 'toskip' value. The new iterator index is automatically stored, returning to
 * it first value if an error append.
...
/*!\fn int strcpy_u( char *from , char *to , NSTRBYTE to_size , char split ,
 * NSTRBYTE *it )
...
int strcpy_u(char *from, char *to, NSTRBYTE to_size, char split, NSTRBYTE *it)
{
    ...
    if (_it == to_size) {
        ...
        to[_it] = '\0';
        n_log(LOG_DEBUG,
              "strcpy_u: not enough space to write %d octet to dest (%d max) , "
              "%s: %d \n",
    ...
    if (from[(*it)] != split) {
        n_log(LOG_DEBUG,
              "strcpy_u: split value not found, written %d octet to dest (%d "
              "max) , %s: %d \n",
              _it, to_size, __FILE__, __LINE__);
        return FALSE;
    }
 ...
 *\brief split the strings into a an array of char *pointer	, ended by a NULL
 * one.
 *\param str The char *str to split
 *\param delim The delimiter, one or more characters
 *\param empty Empty flag. If 1, then empty delimited areas will be added as
 * NULL entries, else they will be skipped.
...
char **split(const char *str, const char *delim, int empty)
{
    ...
    while ((ptr = strstr(largestring, delim)) != NULL) {
        ...
        if (empty == 1 || sizeStr != 0) {
            ...
            Malloc(tab[sizeTab - 1], char,
                   (int) (sizeof(char) * (sizeStr + 1)));
 ...
 *\return a joined string using delimiter with The number of elements or -1 if
 * errors
 */
 ...
 *\param resize_flag Set it to a positive non zero value to allow resizing, or
 * to zero to forbid resizing
 *\return TRUE or FALSE
 */
int nstrcat_ex(N_STR *dest, void *src, NSTRBYTE size, int resize_flag)
{
    ...
    if (dest) {
        if ((dest->written + size + 1) > dest->length && resize_flag == 0) {
            n_log(LOG_ERR,
                  "%p to %p: not enough space. Resize forbidden. %lld needed, "
                  "%lld available",
                  dest, src, dest->written + size + 1, dest->length);
            return FALSE;
        }
    } else {
 ...
 *\brief Append data into N_STR using internal N_STR size and cursor position.
 * Wrapper to nstrcat_ex, with resize_flag on and a block size one byte higher
 * than size.
 *\param dest The N_STR *destination (accumulator)
...
/*!\fn int write_and_fit_ex( char **dest, NSTRBYTE *size, NSTRBYTE *written,
 * const char *src , NSTRBYTE src_size , NSTRBYTE additional_padding )
 *\brief concatenate a copy of src of size src_size to dest, starting at dest[
 * written ], updating written and size variable, allocation of new blocks of
 * (needed size + additional_padding ) if resize is needed. If dest is NULL it
 * will be allocated.
 *\param dest The dest string
 *\param size The current size, will be updated if written + strlen( dest) >
 * size
 *\param written the number of octet added
 *\param src The source string to add
 *\param src_size The source string size
 *\param additional_padding In case the destination is reallocated, number of
 * additional bytes that will be added (provisionning)
...
/*!\fn int write_and_fit( char **dest , NSTRBYTE *size , NSTRBYTE *written ,
 * const char *src )
 *\brief concatenate a copy of src of size strlen( src ) to dest, starting at
 * dest[ written ], updating written and size variable, allocation of new blocks
 * of (needed size + 512) if resize is needed. If dest is NULL it will be
 * allocated.
 *\param dest The dest string
 *\param size The current size, will be updated if written + strlen( dest) >
 * size
...
/*!\fn int scan_dir_ex( const char *dir, const char *pattern , LIST *result ,
    ...
    while ((entry = readdir(dp)) != NULL) {
        ...
        if (stat(newname->data, &statbuf) >= 0) {
            if (S_ISDIR(statbuf.st_mode) != 0) {
                ...
                /* Recurse */
                if (recurse != FALSE) {
                    if (scan_dir_ex(newname->data, pattern, result, recurse,
                                    mode) != TRUE) {
                        n_log(LOG_ERR,
                              "scan_dir_ex( %s , %s , %p , %d , %d ) returned "
                              "FALSE !",
                              newname->data, pattern, result, recurse, mode);
                    }
                }
                free_nstr(&newname);
                continue;
                ...
                if (wildmatcase(newname->data, pattern) == TRUE) {
                    if (mode == 0) {
                        ...
                        if (file) {
                            ...
                            n_log(LOG_ERR, "Error adding %s/%s to list", dir,
                                  entry->d_name);
                        }
                    } else if (mode == 1) {
 ...
 *\brief Written by Rich Salz rsalz at osf.org, refurbished by me. Wildcard
 * pattern matching .
...
/*!\fn char *str_replace ( const char *string, const char *substr, const char
 *replacement )
                  ...
                  const char *replacement)
{
    ...
    while ((tok = strstr(head, substr))) {
        oldstr = newstr;
        Malloc(newstr, char,
               strlen(oldstr) - strlen(substr) + strlen(replacement) + 8);
...
/*!\fn int str_sanitize_ex( char *string, const NSTRBYTE string_len, const char
 * *mask, const NSTRBYTE masklen, const char replacement )
...
{
    return str_sanitize_ex(string, strlen(string), mask, strlen(mask),
                           replacement);
}

/*!fn int nstr_realloc( N_STR *nstr , size_t size )
 * \brief reallocate a nstr internal buffer. Warning: N_STR content will be
 * reset !
