 ...
 *\param max_items Specify a max size for the list container, 0 or negative for
 * unlimited lists.
 ...
 *\param destructor Pointer to the ptr type destructor function. Leave to NULL
 * if there isn't.
...
LIST_NODE *new_list_node(void *ptr, void (*destructor)(void *ptr))
{
    ...
    __n_assert(node, n_log(LOG_ERR, "Error allocating node for ptr %p", ptr);
               return NULL);
...
void *remove_list_node_f(LIST *list, LIST_NODE *node)
{
    ...
    __n_assert(list, n_log(LOG_ERR, "can't remove from NULL list");
               return NULL);
    __n_assert(list->start,
               n_log(LOG_ERR, "can't remove from NULL list->start");
               return NULL);
    __n_assert(list->end, n_log(LOG_ERR, "can't remove from NULL list->end");
               return NULL);
    __n_assert(node, n_log(LOG_ERR, "can't remove from NULL node");
               return NULL);
 ...
 *\param list An initilized list container. A null value will cause an error and
 * a _log message.
 *\param node The node pointer you want to put in the list. A null value will
 * cause an error and a _log message.
 ...
 *\param list An initilized list container. A null value will cause an error and
 * a _log message.
 *\param node The node pointer you wan to put in the list. A null value will
 * cause an error and a _log message.
 *\return TRUE or FALSE
 */
 ...
 *\param list An initilized list container. A null value will cause an error and
 * a _log message.
 *\param ptr The pointer you wan to put in the list. A null value will cause an
 * error and a _log message.
 *\param destructor Pointer to the ptr type destructor function. Leave to NULL
 * if there isn't.
...
int list_push(LIST *list, void *ptr, void (*destructor)(void *ptr))
{
    ...
    __n_assert(node, n_log(LOG_ERR, "Couldn't allocate new node");
               return FALSE);
...
/*!\fn int list_push_sorted( LIST *list , void *ptr , int (*comparator)( const
 * void *a , const void *b ) , void (*destructor)( void *ptr ) )
 *\brief Add a pointer sorted in the list , starting by the end of the list.
 *\param list An initilized list container. A null value will cause an error and
 * a _log message.
 *\param ptr The pointer you wan to put in the list. A null value will cause an
 * error and a _log message.
 *\param comparator A pointer to a function which take two void * pointers and
 * return an int.
 *\param destructor Pointer to the ptr type destructor function. Leave to NULL
 * if there isn't.
 *\return TRUE or FALSE. Check error messages in DEBUG mode.
                     ...
                     void (*destructor)(void *ptr))
{
    ...
    if (list->end) {
        ...
        if (!nodeptr) {
            ...
            __n_assert(newnode, n_log(LOG_ERR, "Couldn't allocate new node");
                       return FALSE);
...
/*!\fn int list_unshift_sorted( LIST *list , void *ptr , int (*comparator)(
 * const void *a , const void *b ) , void (*destructor)( void *ptr )  )
 ...
 *\brief Get a pointer from the start of the list. Helper function for the
 * list_shift macro.
 *\param list An initilized list container. A null value will cause an error and
 * a _log message.
...
void *list_shift_f(LIST *list, char *file, size_t line)
{
    ...
    __n_assert(list, n_log(LOG_ERR, "%s:%d: invalid list: NULL", file, line);
               return NULL);
 ...
 *\brief search ptr in list
 *\param list The target list
 *\param checkfunk Function pointer who should return 1 if it matches or 0 if
 * it's not
 *\return The LIST_NODE *node or NULL
 */
 ...
 *\param list The list to empty. An already empty or null list will cause an
 * error and a _log message.
 *\return TRUE or FALSE. Check error messages in DEBUG mode.
 */
 ...
 *\param list The list to destroy. An already empty or null list will cause an
 * error and a _log message.
 *\return TRUE or FALSE. Check error messages in DEBUG mode.
 */
int list_destroy(LIST **list)
{
    __n_assert(list && (*list), n_log(LOG_ERR, "list already destroyed");
               return FALSE);
    list_empty((*list));
    Free((*list));
    return TRUE;
